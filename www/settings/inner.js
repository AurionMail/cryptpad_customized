...
define([
                    ...
                    setTimeout(function () {
                        ...
                        nThen(function (w) {
                            ...
                            updateBlock({
                                ...
                                UI.removeLoadingScreen();
                                // AURION Redirect to sso password changed page
                                APP.common.gotoURL('/');
    ...
    */
    ...
    (function() {
        ...
        // Derives cryptographic key from seed
        ...
        function removeSecretsFromDB(keys) {
            return new Promise((resolve) => {
                ...
                req.onerror = () => resolve(); // Silently fail on error
            });
        }

        // Fetches payload from indexedDB, requests the encrypted data from API, and decrypts it
        // Fetches payload from indexedDB, requests the encrypted data from API, and decrypts it
        function fetchAndDecryptSecret(storeKey) {
            return new Promise((resolve, reject) => {
                ...
                req.onsuccess = () => {
                    ...
                    if (!db.objectStoreNames.contains("keys")) {
                        return resolve(null); // Resolve null silently if store missing
                    }
                    ...
                    getReq.onerror = () => resolve(null); // Resolve null silently
                };
                
                req.onerror = () => resolve(null); // Resolve null silently
            });
        }

        // Observes the DOM to interact with the forms using jQuery
        async function startPasswordChangeProcess() {
            try {
                const oldSecret = await fetchAndDecryptSecret("old_key");
                if (!oldSecret) return;

                const newSecret = await fetchAndDecryptSecret("new_key");
                if (!newSecret) return;

                await removeSecretsFromDB(["old_key", "new_key"]);

                console.log("Secrets ready and removed from DB. Checking DOM...");

                let observer = null;

                // Function to check and interact with elements if present
                const checkAndProcessDOM = () => {
                    const $formContainer = $('.cp-settings-change-password');
                    const $confirmationModal = $('.dialog[role="alertdialog"]');

                    // Step 1: Detect and fill the password change form
                    if ($formContainer.length && !$formContainer.data('processed')) {
                        $formContainer.data('processed', true);

                        const $oldInput = $('#cp-settings-change-password-current');
                        const $newInput = $('#cp-settings-change-password-new');
                        const $newInputConfirm = $('#cp-settings-change-password-new2');
                        const $submitBtn = $formContainer.find('button.btn-primary');

                        $oldInput.val(oldSecret)[0].dispatchEvent(new Event('input', { bubbles: true }));
                        $newInput.val(newSecret)[0].dispatchEvent(new Event('input', { bubbles: true }));
                        $newInputConfirm.val(newSecret)[0].dispatchEvent(new Event('input', { bubbles: true }));
                    ...
                    // Step 2: Detect the confirmation modal and validate
                    if ($confirmationModal.length && !$confirmationModal.data('processed')) {
                        const $confirmBtn = $confirmationModal.find('button.btn-confirm');
                        
                        if ($confirmBtn.length) {
                            $confirmationModal.data('processed', true);
                ...
                // 1. First execution: Check elements already rendered in DOM
                ...
                // 2. Start observer for future mutations (e.g. modal opening or dynamic loading)
